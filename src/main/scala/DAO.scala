...
object DAO {
  ...
  class Users(tag: Tag) extends Table[User](tag, "users") {
    ...
    def * = (name, email, message, timestamp, id.?) <> (User.tupled, User.unapply)
  }

  case class DUser(name: String, email: String, message: String, timestamp: String, id: Option[Long] = None)
  ...
  class DUsers(tag: Tag) extends Table[DUser](tag, "dusers") {
    ...
    def * = (name, email, message, timestamp, id.?) <> (DUser.tupled, DUser.unapply)
  }
